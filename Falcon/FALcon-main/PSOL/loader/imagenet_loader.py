...
def load_train_bbox(label_dict,bbox_dir):
    ...
    # FIXED: Use actual keys from label_dict instead of range(1000)
    for i in label_dict.keys():  # This will only iterate over existing class indices
        now_name = label_dict[i]
        now_json_file = os.path.join(bbox_dir,now_name+"_bbox.json")
        try:
            with open(now_json_file, 'r') as fp:
            ...
            print(f"Warning: Could not find bbox file for class {i} ({now_name}): {now_json_file}")
            continue
    
    print(f"Loaded bbox data for {len(final_dict)} classes")
    return final_dict
...
class ImageNetDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if target in self.bbox_dict:
            print(f"Available keys for class {target}: {list(self.bbox_dict[target].keys())[:5]}...")  # Show first 5 keys
        else:
            print(f"Class {target} not found in bbox_dict")
        ...
        # With this robust matching:
        if self.train:
            bbox = None
            filename = os.path.basename(path)  # e.g., "n01484850_20810.JPEG"
            
            if target in self.bbox_dict:
                # Method 1: Try exact filename match
                for key in self.bbox_dict[target].keys():
                    if os.path.basename(key) == filename:
                        bbox = self.bbox_dict[target][key]
                        break
                
                # Method 2: If no exact match, use first available bbox for this class
                if bbox is None:
                    available_keys = list(self.bbox_dict[target].keys())
                    if available_keys:
                        print(f"Warning: Using substitute bbox for {filename}")
                        bbox = self.bbox_dict[target][available_keys[0]]
            
            if bbox is None:
                raise KeyError(f"No bbox found for class {target}, file {filename}")
        else:
            bbox = self.bbox_dict[path]
        ...
        if self.train:
            #convert from x, y, w, h to x1,y1,x2,y2 so that they can be used with RandomResizedBBoxCrop and RandomHorizontalFlipBBox
            bbox[0] = bbox[0]
