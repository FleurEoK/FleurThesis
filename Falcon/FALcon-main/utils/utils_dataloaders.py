...
from utils.dataloader_imagenet import ImageNetDataset
from utils.dataloader_voc import VOCDetection
from utils.dataloader_imagenet2013_det_valonly import ImageNetDataset2013_detection
from utils.dataloader_cifar10 import CIFAR10WithBBox

def get_dataloaders(config, loader_type='train'):
    assert config.dataset.lower() in ['celeba', 'imagenet', 'cifar10', 'voc07', 'voc12', 'imagenet2013-det'], "Received unsupported type of dataset!"
    ...
    ### ImageNet dataset
        ...
        if loader_type == 'train_and_val': # split ImageNet train set into train and val sets for hyperparameter search
            ...
            train_loader = torch.utils.data.DataLoader(train_data, batch_size=config.batch_size_eval, shuffle=True, num_workers=4, pin_memory=True)
            return train_loader
    ...
    # CIFAR-10 dataset
        ...
        assert loader_type in ['train', 'valid', 'test', 'all', 'trainval', 'train_and_val'], "Unsupported dataset split!"
        ...
        def load_bbox_dict(json_path):
            if json_path is None:
                return None
            with open(json_path, 'r') as f:
                bbox_dict = json.load(f)
            return {int(k): np.array(v, dtype='float32') for k, v in bbox_dict.items()}


        # Load bbox dicts if available
        pseudo_bbox_dict = load_bbox_dict(config.pseudo_bbox_dir) if hasattr(config, 'pseudo_bbox_dir') else None
        gt_bbox_dict     = load_bbox_dict(config.gt_bbox_dir) if hasattr(config, 'gt_bbox_dir') else None

        if loader_type == 'train_and_val':
            full_dataset = CIFAR10WithBBox(root=config.dataset_dir,
                                        train=True,
                                        transform=transform,
                                        bbox_dict=pseudo_bbox_dict,
                                        input_size=config.full_res_img_size)

            num_samples = len(full_dataset)
            indices = list(range(num_samples))
            split = int(np.floor(config.valid_split_size * num_samples))
            ...
            train_loader = torch.utils.data.DataLoader(full_dataset, batch_size=config.batch_size_train,
                                                    sampler=train_sampler, num_workers=4, pin_memory=True)
            valid_loader = torch.utils.data.DataLoader(full_dataset, batch_size=config.batch_size_train,
                                                    sampler=valid_sampler, num_workers=4, pin_memory=True)
            return train_loader, valid_loader

        elif loader_type == 'train':
            dataset = CIFAR10WithBBox(root=config.dataset_dir,
                                    train=True,
                                    transform=transform,
                                    bbox_dict=pseudo_bbox_dict,
                                    input_size=config.full_res_img_size)
            return torch.utils.data.DataLoader(dataset, batch_size=config.batch_size_train, shuffle=True, num_workers=4, pin_memory=True)

        elif loader_type == 'test':
            dataset = CIFAR10WithBBox(root=config.dataset_dir,
                                    train=False,
                                    transform=transform,
                                    bbox_dict=gt_bbox_dict,
                                    input_size=config.full_res_img_size)
            return torch.utils.data.DataLoader(dataset, batch_size=config.batch_size_eval, shuffle=False, num_workers=4, pin_memory=True)

        else:
            print("Unsupported loader_type for CIFAR-10. Use 'train', 'train_and_val', or 'test'.")
